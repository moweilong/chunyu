...
func (app *App) initializeLogger() {
	...
	if viper.IsSet("log.dir") {
		logOptions.Dir = viper.GetString("log.dir")
	}
	if viper.IsSet("log.id") {
		logOptions.ID = viper.GetString("log.id")
	}
	if viper.IsSet("log.name") {
		logOptions.Name = viper.GetString("log.name")
	}
	if viper.IsSet("log.version") {
		logOptions.Version = viper.GetString("log.version")
	}
	if viper.IsSet("log.debug") {
		logOptions.Debug = viper.GetBool("log.debug")
	}
	if viper.IsSet("log.max_age") {
		logOptions.MaxAge = time.Duration(viper.GetInt("log.max_age"))
	}
	if viper.IsSet("log.rotation_time") {
		logOptions.RotationTime = time.Duration(viper.GetInt64("log.rotation_time"))
	}
	if viper.IsSet("log.rotation_size") {
		logOptions.RotationSize = viper.GetInt64("log.rotation_size")
	}
	if viper.IsSet("log.level") {
		logOptions.Level = viper.GetString("log.level")
	}
	if viper.IsSet("log.tick_sec") {
		logOptions.TickSec = viper.GetInt("log.tick_sec")
	}
	if viper.IsSet("log.first") {
		logOptions.First = viper.GetInt("log.first")
	}
	if viper.IsSet("log.thereafter") {
		logOptions.Thereafter = viper.GetInt("log.thereafter")
	}
